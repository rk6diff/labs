...
int main(int argc, char* argv[]) {
	...
	GC gc[2];
	for (int i = 0; i < 2; ++i) { gc[i] = XCreateGC(dpy, root, 0, 0); }
	...
	for (;;) {
		...
		if (height < DisplayHeight(dpy, screen)) { break; }
		argv[1] = "16x8";
	}
	...
	unsigned long emask = (ExposureMask | KeyPressMask | FocusChangeMask |
						   VisibilityChangeMask | ControlMask);
	...
	while (multi != 0) { /* Async dispatch event with multic ground */
		...
		if (is_inc == false && (circle.radius <= circle.radius_min)) {
			is_inc = true;
		}

		twist(dpy, window, gc[g], &circle, &is_inc);
		XDrawString(dpy, window, gc[g], 8, height, "CTRL-C", strlen("CTRL-C"));
	}
	rep5355(dpy, AutoRepeatModeOn);

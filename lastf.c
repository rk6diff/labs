...
int redraw(XEvent* ev, GC gc, XPiAr2* pr) {
	...
	XDrawString(ev->xexpose.display, ev->xexpose.window, gc, 8, y, "CTRL-R", 6);
...
int twist(Display* dpy, Window win, GC gc, XPiAr2* pr) {
	recent(pr);
	XFillArc(dpy, win, gc, pr->c.x - pr->R, pr->c.y - pr->R, (2 * pr->R),
			 (2 * pr->R), 0, 64 * 360);
	XFlush(dpy);
	return (pr->R);
} /* twist */
...
int overlap(XEvent* ev) {
	if (ev->xvisibility.state != VisibilityUnobscured) { return true; }
	return false;
} /* overlap */
